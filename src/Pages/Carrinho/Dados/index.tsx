import { Box, Button, FormControl, FormHelperText, Grid, OutlinedInput, Stack, Typography, useFormControl } from "@mui/material"
import { useMemo, useState } from "react"
import { IDadosPessoais, usePerfilContext } from "../../../Common/Context/DadosPessoais";
import { Title } from "../../../components/Title"



// inpute Helper
const MyFormHelperText = ({ helperFocused, helper }: { helperFocused?: string, helper?: string }) => {
    const { focused } = useFormControl() || {};

    const helperText = useMemo(() => {
        if (focused) {
            return helperFocused
        };
        return helper || ''
    }, [focused])
    return <FormHelperText>{helperText}</FormHelperText>
}



export const MyInput = ({ label, helperFocused, helper, readOnly, register }: { label: string, helperFocused?: string, helper?: string, readOnly: boolean, register: any }) => {
    const { setPerfil, perfil, adicionaDadosAoPerfil, handleSubmit, onSubmit } = usePerfilContext()


    return (
        <FormControl sx={{ width: '100%' }}>
            <OutlinedInput inputProps={{
                readOnly: readOnly,
            }}
                {...register(label)}
                label={label}
                fullWidth
                placeholder={label}
            />
            <MyFormHelperText helperFocused={helperFocused} helper={helper} />
        </FormControl>
    )
}


export const FormularioCarrinho = () => {
    const { register, handleSubmit, onSubmit } = usePerfilContext()
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Title text='dados pessoais' />
            <Stack spacing={.8} >
                <MyInput register={register} readOnly={false} helperFocused='insira seu nome' label='nome' helper=' ' />
                <MyInput  register={register} readOnly={false} helperFocused='insira seu último sobrenome' label='sobrenome' helper=' ' />
                <MyInput register={register} readOnly={false} helperFocused='insira seu telefone' label='telefone' helper='(00) 0.0000-0000' />
                <MyInput register={register} readOnly={false} helperFocused='insira seu cep' label='cep' helper='00.000-000' />
            </Stack>

            <Title text='endereço' />

            <Grid container columnSpacing={2} >
                <Grid item xs={6}>
                    <MyInput register={register} readOnly={false} helperFocused='rua' label='rua' helper=' ' />
                </Grid>
                <Grid item xs={6}>
                    <MyInput register={register} readOnly={false} helperFocused='bairro' label='bairro' helper=' ' />
                </Grid>
                <Grid item xs={6}>
                    <MyInput register={register} readOnly={false} helperFocused='cidade' label='cidade' helper=' ' />
                </Grid>
                <Grid item xs={6}>
                    <MyInput register={register} readOnly={false} helperFocused='estado' label='estado' helper=' ' />
                </Grid>
            </Grid>

            <Box sx={{ bgcolor: 'transparent', width: '100%', height: 75 }}>

                <Button type='submit' sx={{ p: 3.2, width: '100%', borderRadius: 0 }}>
                    <Typography>finalizar pedido</Typography>
                </Button>
            </Box>
        </form>
    )
}